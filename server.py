from fastapi import FastAPI, Body, Depends, Path, status, HTTPException
from fastapi.responses import RedirectResponse
from core.config import PSQL_URL, HOST, PORT
...
async def createURLs(request: schemas.createShortURL = Body(..., embed=False) , db: Session = Depends(get_db)):
    new_short_url = crud.create_url(db, request)
    return new_short_url.__dict__

@app.get('/{s_url}', status_code=status.HTTP_301_MOVED_PERMANENTLY, tags=['Redirect'])
async def redirectURL(s_url:str = Path(..., min_length=8, max_length=8), db: Session = Depends(get_db)):
    short_url = crud.single_url(db, s_url)
    crud.update_url(db, s_url, {'counter': short_url.counter+1})
